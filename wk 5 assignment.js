...
class IceCream { //second class
    ...
    addFlavor(flavor) {
        if (flavor instanceof Flavor) { //instanceof opereator checks to see if flavor is actually an instance of flavor class
    ...
    describe() { //prints the ice cream brand and flavor information
...
start() { //is the start or entry point of menu application
    let selection = this.showMainMenuOptions(); //selection beginning, builds menu methods and returns user selection

    while (selection != 0) { //a variabe to get user imput of which option in the menu the user selected
        switch (selection) { //what user selects and do something based off of what was selected
            case '1':
                this.createIceCream(); 
                break;  
                case '2':
        ...
        selection = this.showMainMenuOptions(); //outside of switch, selection while still inside of loop, keeps looping as long as 0 or something other than 1-4 are not selected
...
createIceCream() {
    let brand = prompt('Enter brand for new ice cream: '); //prompts the user for brand of ice cream
    this.icecreams.push(new IceCream(brand)); //creates a new brand, passes the brand into the ice cream that will be pushed to ice cream array
}

viewIceCream() { 
    let index = prompt('Enter the index of the ice cream you wish to view'); //asks the user which index of the brand they want to view
    if (index > -1 && index < this.icecreams.length) { //validates user imput
     ...
     for (let i = 0; i < this.selectedIceCream.flavors.length; i++) { //flavors array
       description += i + ') ' + this.selectedIceCream.flavors[i].brand 
        + ' - ' + this.selectedIceCream.flavors[i].price + '\n'; //prints index, selected.icecream.flavors array and i is specific flavor for this iteration and the brand
     }
 ...
 deleteIceCream() {
    let index = prompt('Enter the index of the ice cream you wish to delete:'); //user can delete index of brand 
    if (index > -1 && index < this.icecreams.length) {
        this.icecreams.splice(index, 1);
    }
 }

 createFlavor() { //add outside of method, not added within another method unless needs to be, still add inside class to have access to it
    let name = prompt('Enter name for new flavor:'); //user enters flavor, have an undefined error 
    let price = prompt('Enter price for new flavor:'); //user enters price
    this.selectedIceCream.flavors.push(new Flavor(name, price)); //know what flavor is selected, pass name and price from user 
 }
